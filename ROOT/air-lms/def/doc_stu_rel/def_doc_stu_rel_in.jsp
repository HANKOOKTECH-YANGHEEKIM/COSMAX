...
<%
	...
	String docStrStr	= "T|임시저장^A|승인^R|부결^S|처리";
...
var getStuCode = function(targetId, val){
	var data = {};
	data["PARENT_CODE_ID"] = "LMS_"+val+"_STU";
...
<script id="inListTmpl" type="text/html">
	<tr>
		...
		<td style="text-align:center">
			<select data-type="search" style="width:100%;" id="DOC_STU" onChange="goModSel('\${DOC_STU}','\${UUID}',this,'WRITE_IN_PROC')">
				<option value="" {{if DOC_STU == '' }}selected="selected"{{/if}}>선택</option>
				<option value="T" {{if DOC_STU == 'T' }}selected="selected"{{/if}}>임시저장</option>
				<option value="A" {{if DOC_STU == 'A' }}selected="selected"{{/if}}>승인</option>
				<option value="R" {{if DOC_STU == 'R' }}selected="selected"{{/if}}>부결</option>
				<option value="S" {{if DOC_STU == 'S' }}selected="selected"{{/if}}>처리</option>
			</select>
		</td>
		<td style="text-align:center">
...
<form name="form1" method="post">
			<table>
				...
				<tr>				
					...
					<th >처리구분</th>						
					<td >
						<%=HtmlUtil.getSelect(request, true, "doc_stu", "doc_stu", "|선택^"+docStrStr, "", "class=\"select\" data-type=\"search\"  data-type=\"search\"") %>
<%-- 						<%=HtmlUtil.getSelect(request, true, "stu_cod", "sch_stu_cod", "|-- 선택 --", "", "class=\"select\" data-type=\"search\"  data-type=\"search\"") %> --%>
<!-- 						<input type="hidden" name="stu_cod_nm" id="stu_cod_nm" value=""/> -->
					</td>
					<th>권한자</th>
...
<form name="saveForm" id="saveForm" method="post" onsubmit="return false;">
	...
	<tr>
		...
		<td style="width:35%;">
			<%=HtmlUtil.getSelect(request, true, "stu_gbn", "stu_gbn", schLmsGbnStr, "", "class=\"select\" onChange=\"getStuCode('stu_cod', this.value);\" data-type=\"search\"") %>
		</td>
		<th style="width:10%;">처리구분</th>						
		<td style="width:35%;">
				<%=HtmlUtil.getSelect(request, true, "DOC_STU", "DOC_STU", docStrStr, "A", "class=\"select\" data-type=\"search\" ") %>
<%-- 			<%=HtmlUtil.getSelect(request, true, "stu_cod", "stu_cod", "|-- 선택 --", "", "class=\"select\" onChange=\"$('#stu_cod_nm').val($('#stu_cod :selected').text())\" data-type=\"search\"") %> --%>
<!-- 			<input type="hidden" name="stu_cod_nm" id="stu_cod_nm" value=""/> -->
		</td>
