...
<%} %>
		...
		<tr>
	        ...
	        <td class="td4">
	            ...
	            function searchSusinja() {
	            	var callback = "changeSusinja(\'[BYEONHOSA_ID]\', \'[BYEONHOSA_NAM]\', \'[SOSOG_COD]\', \'[SOSOG_NAM]\')";
	            	
	            	airLms.popupWiimSusinjaSelects(callback, '<%=sol_mas_uid%>', 'N');
...
// 	            	airCommon.popupUserSelect("changeChamjoja", param);
	            ...
	            function changeSusinja(user_id, user_name, sosog_code, sosog_name) {
	            	
	                user_id = user_id.replace(/\|\|/g,",");
	                user_name = user_name.replace(/\|\|/g,",");
	                sosog_code = sosog_code.replace(/\|\|/g,",");
	                sosog_name = sosog_name.replace(/\|\|/g,",");
	                
	                var init_user_id 	= "";//document.getElementById("susinja_id").value;	
	            	var init_user_name 	= "";//document.getElementById("susinja_nam").value;
	            	var init_sosog_code = "";//document.getElementById("sosog_id").value;	
	            	var init_sosog_name = "";//document.getElementById("sosog_nam").value;
	            	
	            	if(init_user_id.indexOf(user_id) == -1){
		            	init_user_id = (init_user_id == "" ? user_id : init_user_id +","+ user_id);
		            	init_user_name = (init_user_name == "" ? user_name : init_user_name +","+ user_name);
		            	init_sosog_code = (init_sosog_code == "" ? sosog_code : init_sosog_code +","+ sosog_code);
		            	init_sosog_name = (init_sosog_name == "" ? sosog_name : init_sosog_name +","+ sosog_name);
		            	
		            	initSusinja(true, init_user_id, init_user_name, init_sosog_code, init_sosog_name);
	            	}            	            	
	            }
	            
	            function initSusinja(isForceInit, user_id, user_name, sosog_code, sosog_name) {
	            	user_id = (user_id == undefined ? "" : user_id);
	            	user_name = (user_name == undefined ? "" : user_name);
	            	sosog_code = (sosog_code == undefined ? "" : sosog_code);
	            	sosog_name = (sosog_name == undefined ? "" : sosog_name);
	            	
	            	if (!isForceInit && !confirm("<%=StringUtil.getLocaleWord("M.초기화하시겠습니까",siteLocale,StringUtil.getLocaleWord("L.수신자", siteLocale))%>")) {
	            		return false;
	            	}
	            		
	            	document.getElementById("susinja_id").value = user_id;
	            	document.getElementById("susinja_nam").value = user_name;
	            	document.getElementById("sosog_id").value = sosog_code;
	            	document.getElementById("sosog_nam").value = sosog_name;
	            	$("#susinja_nam").val().replace("\n", '<br>');
	            }                        
	            </script>
	            <span style="float: left">
		            <span class="ui_btn small icon"><span class="cancel"></span><a href="javascript:void(0)" onclick="initSusinja(false)"><%=StringUtil.getLocaleWord("B.INIT",siteLocale)%></a></span>
		            <span class="ui_btn small icon"><span class="search"></span><a href="javascript:void(0)" onclick="searchSusinja()"><%=StringUtil.getLocaleWord("B.SEARCH",siteLocale)%></a></span>
	           	</span><br />
	            <textarea name="susinja_nam" id="susinja_nam" readonly="readonly"  class="textarea width_max" rows="5"><%=StringUtil.convertForInput(masMap.getString("SUSINJA_NAM")) %></textarea>
<%-- 	            <input type="text" class="text width_max" name="susinja_nam" id="susinja_nam" readonly="readonly" value="<%=StringUtil.convertForInput(masMap.getString("SUSINJA_NAM")) %>"  /> --%>
	            <input type="hidden" id="sosog_id" name="sosog_id" value="<%=StringUtil.convertForInput(masMap.getString("SOSOG_ID")) %>">
	            <input type="hidden" id="sosog_nam" name="sosog_nam" value="<%=StringUtil.convertForInput(masMap.getString("SOSOG_NAM")) %>">
	            <input type="hidden" id="susinja_id" name="susinja_id" value="<%=StringUtil.convertForInput(masMap.getString("SUSINJA_ID")) %>">
	        </td>
	        <th class="th4"><%=StringUtil.getLocaleWord("L.참조자", siteLocale) %></th>
	        <td class="td4">
	            <script type="text/javascript"> 
	            function searchChamjoja() {
	            	var defaultUser = $("#chamjoja_cod").val();
					var param ={};
	            ...
	            function initChamjoja(isForceInit, code, name, name_ko, name_en) {
	            	...
	            	document.getElementById("chamjoja_cod").value = code;
	            	document.getElementById("chamjoja_nam").value = name;
	            	document.getElementById("chamjoja_nam_ko").value = name_ko;
	            	document.getElementById("chamjoja_nam_en").value = name_en;
	            	 
	            	$("#chamjoja_nam").val().replace("\n", '<br>');
	            	$("#chamjoja_nam_ko").val().replace("\n", '<br>');
	            	$("#chamjoja_nam_en").val().replace("\n", '<br>');
	            }                        
	            </script>
	            <input type="hidden" id="chamjoja_cod" name="chamjoja_cod" value="<%=StringUtil.convertForInput(masMap.getDefStr("CHAMJOJA_COD","")) %>">
	            <input type="hidden" id="chamjoja_nam_ko" name="chamjoja_nam_ko" value="<%=StringUtil.convertForInput(masMap.getString("CHAMJOJA_NAM_KO")) %>">
	            <input type="hidden" id="chamjoja_nam_en" name="chamjoja_nam_en" value="<%=StringUtil.convertForInput(masMap.getString("CHAMJOJA_NAM_EN")) %>">
	            <span style="float:left;"> 
	            ...
	            <textarea name="chamjoja_nam" id="chamjoja_nam" readonly="readonly"  class="width_max"  rows="5"><%=StringUtil.convertForInput(masMap.getDefStr("CHAMJOJA_NAM","")) %></textarea>
...
<div class="buttonlist">
    <div class="left">
    	<span style="color:red;"><%=StringUtil.getLocaleWord("M.외부변호사업무연락_코멘트", siteLocale) %></span>
    </div>
    <div class="rigth">
