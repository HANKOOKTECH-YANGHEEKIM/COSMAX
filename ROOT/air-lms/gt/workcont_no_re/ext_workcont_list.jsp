...
%>
<form name="extWorkContFormNore<%=sol_mas_uid%>" style="margin:0; padding:0;">
	<input type="hidden" name="sol_mas_uid" data-type="search" value="<%=sol_mas_uid%>"/>
</form>
<table class="list" id="TB_EXT_WORKCONT_NORE<%=sol_mas_uid%>">
	<caption>
		<span class="left" style="color:darkred;">※ 본 탭은 법무팀만 볼 수 있습니다.</span>
		<span class="right">
			<%if(isAuths){ %>
	    	<span class="ui_btn medium icon"><span class="write"></span><a href="javascript:void(0)" onclick="goNewExtWorkContNoreWrite<%=sol_mas_uid%>();"><%=StringUtil.getLocaleWord("B.REG",siteLocale)%></a></span>
			<%} %>
		</span>
	</caption>
	<thead>
	<tr>
		...
		<th style="width:auto" data-opt='{"align":"left","col":"YOCHEONG_TIT","onClick":"popupExtWorkNoreCont<%=sol_mas_uid%>(\"@{GT_EXT_WORKCONT_UID}\")"}'><%=StringUtil.getLocaleWord("L.제목", siteLocale) %></th>
		...
		<th style="width:10%" data-opt='{"align":"center","col":"LIST_SUSINJA_NAM"}'><%=StringUtil.getLocaleWord("L.수신자", siteLocale) %></th>
		<th style="width:10%" data-opt='{"align":"center","col":"SUSIN_DTE"}'><%=StringUtil.getLocaleWord("L.회신일", siteLocale) %></th>
	</tr>
	</thead>
	<tbody id="TB_EXT_WORKCONT_NORE<%=sol_mas_uid%>Body"></tbody>

</table>
<%-- 페이지 목록 --%>
<div class="pagelist" id="TB_EXT_WORKCONT_NORE<%=sol_mas_uid%>Page"></div> 

<script>
<%if(isAuths){ %>
var goNewExtWorkContNoreWrite<%=sol_mas_uid%> = function(){
	var url = "/ServletController";
	url += "?AIR_ACTION=LMS_GT_WORKCONT_NO_RE";
...
var popupExtWorkNoreCont<%=sol_mas_uid%> = function(gt_ext_workcont_uid){
...
var getLmsGtExtWorkNoreCont<%=sol_mas_uid%> = function(pageNo){
	if(pageNo == undefined) pageNo =1;
	var data = airCommon.getSearchQueryParams(document.extWorkContFormNore<%=sol_mas_uid%>);
	...
	airCommon.callAjax("<%=actionCode%>", "JSON_LIST",data, function(json){
		airCommon.createTableRow("TB_EXT_WORKCONT_NORE<%=sol_mas_uid%>", json, pageNo, 10, "getLmsGtExtWorkNoreCont<%=sol_mas_uid%>");
	});
};

$(function(){
	getLmsGtExtWorkNoreCont<%=sol_mas_uid%>(1);
});
</script>