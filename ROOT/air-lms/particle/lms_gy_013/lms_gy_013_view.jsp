...
<%
	...
	String LMS_GY_SKIP_REASON_CODESTR = StringUtil.getCodestrFromSQLResults(resultMap.getResult("LMS_GY_SKIP_REASON"), "CODE_ID,LANG_CODE","|"+StringUtil.getLocaleWord("L.CBO_SELECT", siteLocale));
...
<table class="basic">
	...
	<tr>
	    <th class="th2" nowrap="nowrap"><%=StringUtil.getLocaleWord("L.계약명",siteLocale)%></th>
		<td class="td2" colspan ="3"><%=StringUtil.convertForView(jsonMap.getString("LMS_PATI_GYEYAG_TIT"))%></td>
    </tr>
    <tr>			
 		<th class="th4" nowrap="nowrap"><%=StringUtil.getLocaleWord("L.회사",siteLocale)%></th>
		<td class="td4">
	...
	<tr>
		<th class="th4" nowrap="nowrap"><%=StringUtil.getLocaleWord("L.의뢰부서",siteLocale)%></th>
		<td class="td4"><%=jsonMap.getStringView("LMS_PATI_YOCHEONG_DPT_NAM") %></td>
		<th class="th4" nowrap="nowrap"><%=StringUtil.getLocaleWord("L.의뢰자",siteLocale)%></th>
		<td class="td4">
	...
	<tr>
		<th class="th4" nowrap="nowrap"><%=StringUtil.getLocaleWord("L.유형",siteLocale)%></th> 
		<td class="td4"><%=StringUtil.getCodestrValue(request,jsonMap.getString("LMS_PATI_YUHYEONG"), sYuhyeong_list)%></td> 
		<th class="th4" nowrap="nowrap"><%=StringUtil.getLocaleWord("L.계약기간",siteLocale)%></th>
		<td class="td4">
	...
	<tr>
		<th class="th4" nowrap="nowrap"><%=StringUtil.getLocaleWord("L.계약체결일자",siteLocale)%></th>
		<td class="td4">
			<%=StringUtil.convertForView(jsonMap.getString("LMS_PATI_CHEGYEOL_DTE"))%>			
		</td>
		<th class="th4" nowrap="nowrap"><%=StringUtil.getLocaleWord("L.계약금액",siteLocale)%></th>
		<td class="td4" style="text-align:right;" nowrap="nowrap">
	 ...
	 <tr>
		<th class="th4" nowrap="nowrap"><%=StringUtil.getLocaleWord("L.계약상대방",siteLocale)%></th>
		<td class="td4">
			<%=StringUtil.convertForView(jsonMap.get("LMS_PATI_GYEYAG_SANGDAEBANG_NAM"))%>
		</td>
		<th class="th4" nowrap="nowrap"><%=StringUtil.getLocaleWord("L.상대방",siteLocale)%> <%=StringUtil.getLocaleWord("L.상세",siteLocale)%></th>
		<td class="td4">
			<%=StringUtil.convertForView(jsonMap.get("LMS_PATI_GYEYAG_SANGDAEBANG_DETAIL"))%>
		</td>
	</tr>
	<tr>
		<th class="th2" nowrap="nowrap"><%=StringUtil.getLocaleWord("L.신규_수정_갱신",siteLocale)%></th>
	...
	<tr>
		<th class="th4" nowrap="nowrap"><span><%=StringUtil.getLocaleWord("L.품목",siteLocale)%></span></th>
		<td class="td4" colspan="3">
			<%=StringUtil.convertForView(jsonMap.get("LMS_PATI_ITEM"))%>
		</td>
	</tr>
	<tr>
		<th class="th4" nowrap="nowrap"><%=StringUtil.getLocaleWord("L.법무검토_생략_사유",siteLocale)%></th>
		<td class="td4" colspan="3" nowrap="nowrap">
			<%=HtmlUtil.getSelect(request, false, "lms_pati_skip_reason_cod", "lms_pati_skip_reason_cod", LMS_GY_SKIP_REASON_CODESTR, jsonMap.getString("LMS_PATI_SKIP_REASON_COD") ,  "onChange=\"setSkipInput(this.value);\" class=\"select\"").replace("C.", "")%>
			<span style="display:<%="SKIP_REASON_ETC".equals(jsonMap.getStringView("LMS_PATI_SKIP_REASON_COD"))?"":"none" %>"><br/>(<%=jsonMap.getString("LMS_PATI_SKIP_REASON_ETC")%>)</span>
		</td>
	</tr>
	<tr>
		...
		<th class="th4" nowrap="nowrap"><%=StringUtil.getLocaleWord("L.참조자_열람가능",siteLocale)%></th>
		<td class="td4">
			<table style="width:100%;">
			<%
    ...
    <tr>
		<th class="th2" nowrap="nowrap"><%=StringUtil.getLocaleWord("L.품의내용",siteLocale)%></th>
		<td class="td2" colspan="3">
	...
	<tr>
		<th class="th4" nowrap="nowrap"><%=StringUtil.getLocaleWord("L.계약서",siteLocale) %> </th>
		<td class="td4">
		...
		<th class="th4" nowrap="nowrap"><%=StringUtil.getLocaleWord("L.관련자료",siteLocale) %> </th>
		<td class="td4">
			<jsp:include page="/ServletController" flush="true">
				<jsp:param value="SYS_ATCH" name="AIR_ACTION" />
				<jsp:param value="FILE_VIEW" name="AIR_MODE" />
