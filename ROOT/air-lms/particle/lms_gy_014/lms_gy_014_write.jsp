...
<%
	...
	*/
	...
	if("".equals(lms_pati_gy_014_uid)){
		lms_pati_gy_014_uid = StringUtil.getRandomUUID();
	}
...
<script type="text/javascript">
	...
	function <%="Parti"+mng_pati_uid%>_dataCheck(){
		...
		<%if(LmsUtil.isBeobmuTeamUser(loginUser)){ %>
  		...
  		if ("" == frm.lms_pati_chegyeol_dte.value ) {
 			alert ("<%=StringUtil.getScriptMessage("M.필수_입력사항_입니다",siteLocale,StringUtil.getLocaleWord("L.계약체결일자", siteLocale))%>");
...
// 	            	var callback = "changesetYocheong";
	            ...
	            function setReq(data){
	            	if(data){
	            		...
	            		alert(data[0].GROUP_CODE);
	            		alert(data[0].GROUP_NAME_KO);
...
<%-- 		<input type="hidden" name="lms_pati_yocheong_dpt_nam" value="<%=jsonMap.getDefStr("LMS_PATI_YOCHEONG_DPT_NAM_KO", loginUser.getGroupNameKo()) %>" /> --%>
	...
	<tr>
		<th class="th4"><span class="ui_icon required"></span><%=StringUtil.getLocaleWord("L.계약체결일자",siteLocale)%></th>
		<td class="td4">
...
<%--             <br/><span style="color: red"><%=StringUtil.getLocaleWord("M.참조자_선택_알림",siteLocale) %></span> --%>
	...
	<tr>
		<th class="th4"><span class="ui_icon required"></span><%=StringUtil.getLocaleWord("L.계약서",siteLocale) %><br/><span style="color:red;"><%=StringUtil.getLocaleWord("M.날인한계약서",siteLocale) %></span></th>
		<td class="td4">
