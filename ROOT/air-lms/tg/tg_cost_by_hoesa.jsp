...
<form name="form1" method="post">
		...
		<td class="body">
			<table>
				...
				<tr>
					...
					<td><%=HtmlUtil.getSelect(request, true, "schHoesaCd", "schHoesaCd", HOESA_CODESTR, requestMap.getString("SCHHOESACD"), "style=\"width:100%;\" class=\"select\"")%></td>  
	...
	<div class="buttonlist">
		...
		<div class="right">
			...
			function doExcelDown(frm){
				...
				frm.<%=CommonConstants.MODE_CODE%>.value = "COST_BY_HOESA_JSON_EXCEL";
	...
	<table class="basic" id="costTable">
		<tr>
			...
			<th style="text-align:center"><%=StringUtil.getLocaleWord("L.회사",siteLocale) %></th>
	...
	if(COST_LIST != null && COST_LIST.getRowCount() > 0){
		...
		for(int i=0; i< COST_LIST.getRowCount(); i++){ 
           ...
           if(sumRow.equals(COST_LIST.getString(i, "GBN"))||
        		   sumRow.equals(COST_LIST.getString(i, "JIGEUB_GUBUN"))){
        	%>
        	        <tr>
            <td style="text-align:center;"><%=rowCnt %></td>
			<td style="text-align:center;"><%=COST_LIST.getString(i, "HOESA_COD") %></td>
...
function doReset(frm) {
	if (confirm("<%=StringUtil.getScriptMessage("M.ALERT_RESETSEARCHITEMS",siteLocale) %>")) {
		...
		$('#schMonth').val("<%=Calendar.getInstance().get(Calendar.MONTH) %>");
		$("#schYuHyeong").val("");
		$("#schHoesaCd").val("");
	}
}
