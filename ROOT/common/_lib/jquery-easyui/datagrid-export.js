(function($){
    ...
    function toHtml(target, rows){
        ...
        for(var i=0; i<fields.length; i++){
            ...
            var tdStyle = tdStyle0 + ';background:#ececec;width:'+airCommon.convertForView(col.boxWidth)!=""?col.boxWidth:col.width+'px;';
            //-- 강세원 커스터마이징 hidden 필드는 다운로드 하지 않는다.
            if(col.hidden != true){
            	 if(col.toExcel != false){
            		 data.push('<th style="'+tdStyle+'">'+col.title+'</th>');
            	 }
            }else if(col.toExcel == true){
            	data.push('<th style="'+tdStyle+'">'+col.title+'</th>');
            }
        }
        data.push('</tr>');
        $.map(rows, function(row){
            ...
            for(var i=0; i<fields.length; i++){
                ...
                if(col.hidden != true){
                	 if(col.toExcel != false){
                		 data.push(
                				 '<td style="'+tdStyle0+'">'+airCommon.convertForView(row[field])+'</td>'
                		 );
                	 }
                }else if(col.toExcel == true){
                	data.push(
