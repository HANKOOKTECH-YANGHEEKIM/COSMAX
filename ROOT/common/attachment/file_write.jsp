...
function _attachFileList<%=ctrlUuid%>_AddNode(uuid, fileSaveName, fileName, fileSize, defaultFileUid,file_conv_url) {
    ...
    if (file_conv_url == undefined || file_conv_url == null) { file_conv_url = ""; }
    ...
    node_html       += "<input type='hidden' name='_attachFileSaveName<%=ctrlUuid%>' value='"+ airCommon.convertForInput(fileSaveName) +"' />";
    node_html       += "<input type='hidden' name='_attachFileName<%=ctrlUuid%>' value='"+ airCommon.convertForInput(fileName) +"' />";
    ...
    <%--
    var fileUid = document.createElement("input");
    fileUid.type = "hidden";
    fileUid.name = "_attachFileUuid<%=ctrlUuid%>";
    fileUid.value = uuid;

    var saveNam = document.createElement("input");
    saveNam.type = "hidden";
    saveNam.name = "_attachFileSaveName<%=ctrlUuid%>";
    saveNam.value = airCommon.convertForInput(fileSaveName);

    var fileNam = document.createElement("input");
    fileNam.type = "hidden";
    fileNam.name = "_attachFileName<%=ctrlUuid%>";
    fileNam.value = airCommon.convertForInput(fileName);

    var fileSizeEle = document.createElement("input");
    fileSizeEle.type ="hidden";
    fileSizeEle.name = "_attachFileSize<%=ctrlUuid%>";
    fileSizeEle.value = fileSize;

    node_obj.appendChild(fileUid);
    node_obj.appendChild(saveNam);
    node_obj.appendChild(fileNam);
    node_obj.appendChild(fileSizeEle);

    <%if(!"Y".equals(viewCase)){%>
        var viewChk = document.createElement("input");
        viewChk.type = "checkbox";
        viewChk.name = "_attachFileCheck<%=ctrlUuid%>";
        viewChk.value = "Y";
        node_obj.appendChild(viewChk);
    <%}%>
    <%if("Y".equals(showImage)){%>
        var radioChk = document.createElement("input");
        radioChk.type = "radio";
        radioChk.name = "_attachFileRadio<%=ctrlUuid%>"
        radioChk.value = airCommon.convertForInput("/ServletController?AIR_ACTION=SYS_ATCH&AIR_MODE=FILE_SRC&type_code=<%=typeCode%>&file_savename="+fileSaveName);
        radioChk.setAttribute("onclick","javascript:showImage(this.value)");
        node_obj.appendChild(radioChk);
    <%}%>

    if (uuid == "" && defaultFileUid == "") {
        var fileNamNode = document.createTextNode(fileName);
        node_obj.appendChild(fileNamNode);
    } else {
        var download_uid = (uuid != "" ? uuid : defaultFileUid);
        var aTag = document.createElement("a");
        var fileNamNode = document.createTextNode(fileName);
        aTag.setAttribute("href" , "javascript:airCommon.popupAttachFileDownload('"+ download_uid +"');");
        aTag.appendChild(fileNamNode);
        node_obj.appendChild(aTag);
    }
     --%>
...
function _attachFileAddButton<%=ctrlUuid%>_Click() {
	...
	url += "&callbackFunction="+ escape("_attachFileList<%=ctrlUuid%>_AddNode(\'\', \'[FILE_SAVENAME]\', \'[FILE_NAME]\', \'[FILE_SIZE]\', '', \'[FILE_CONV_URL]\')"); //XSS필터링 통과를 위해 escape 적용!
	...
	airCommon.openWindow(url, '800', '450', 'popupAttachJQueryFileUpload', "no", "no");
...
if(!"Y".equals(attachAfterAddMode)){
    ...
    if (viewResult != null && viewResult.getRowCount() > 0) {
        for (int i = 0; i < viewResult.getRowCount(); i++) {
            out.println("_attachFileList"+ ctrlUuid + "_AddNode('"+ viewResult.getString(i, "uuid") +"', '"+ StringUtil.convertForJavascript(viewResult.getString(i, "file_savename")) +"', '"+ StringUtil.convertForJavascript(viewResult.getString(i, "file_name")) +"', '"+ viewResult.getString(i, "file_size") +"','', '"+ StringUtil.convertNullDefault(viewResult.getString(i, "org_file_savename"), viewResult.getString(i, "file_conv_url")) +"');");
        }
    }
    else
    //-- 디폴트 첨부가 있으면 보여줌
    if (defaultViewResult != null && defaultViewResult.getRowCount() > 0) {
        for (int i = 0; i < defaultViewResult.getRowCount(); i++) {
            out.println("_attachFileList"+ ctrlUuid + "_AddNode('', '"+ StringUtil.convertForJavascript(defaultViewResult.getString(i, "file_savename")) +"', '"+ StringUtil.convertForJavascript(defaultViewResult.getString(i, "file_name")) +"', '"+ defaultViewResult.getString(i, "file_size") +"', '"+ defaultViewResult.getString(i, "uuid") +"', '"+ StringUtil.convertNullDefault(defaultViewResult.getString(i, "org_file_savename"), defaultViewResult.getString(i, "file_conv_url"))+"');");
        }
    }
